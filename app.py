...
def show_venue(venue_id):
  ...
  upcoming_shows_data = db.session.query(Artist).join(Show).filter(Show.venue_id==venue_id).filter(Show.start_time>=datetime.now()).all()
  past_shows_data = db.session.query(Artist).join(Show).filter(Show.venue_id==venue_id).filter(Show.start_time<datetime.now()).all()
